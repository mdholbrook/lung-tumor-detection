...
class BSplineDeformation(object):
	...
	def __call__(self,sample):
		...
		for n in range(len(image)):
			...
			domain_physical_dimensions = [image[n].GetSize()[0]*image[n].GetSpacing()[0],
										  image[n].GetSize()[1]*image[n].GetSpacing()[1],
										  image[n].GetSize()[2]*image[n].GetSpacing()[2]]

			bspline = sitk.BSplineTransform(3, spline_order)
			bspline.SetTransformDomainOrigin(image[n].GetOrigin())
			bspline.SetTransformDomainDirection(image[n].GetDirection())
			...
			import pdb
			pdb.set_trace()
			image[n] = sitk.Resample(image[n], bspline)
			label[n] = sitk.Resample(label[n], bspline)
...
class NormalizationHU(object):
	"""
	Images are in HU. Reduce magnitude by 1000th
	"""

	def __init__(self):
		self.name = 'NormalizationHU'

	def __call__(self, sample):
		#divideFilter = sitk.DivideImageFilter()
		#image = divideFilter.Execute(sample['image'], 1000.0)
		image = sample['image']

		for channel in range(len(image)):
			im = image[channel]
			image[channel] = sitk.Divide(im, 1000)
